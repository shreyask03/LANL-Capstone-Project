...
# === Geometry ===
L, W, H = 0.724, 0.414, 0.203

# cant angle
theta = np.radians(15)  # cant angle from horizontal
thetav = np.radians(30) # vertical cant angle
...
T = 44.4  # [N] from Blue Robotics datasheet
Q = 0.017 * rho * (n_max**2) * (D**5)  # keep same torque formula

theta = np.radians(15) # horizontal cant
thetav = np.radians(30) # vert cant
ch = np.cos(theta)  # 0.7071
chy =np.cos(np.pi / 2 - theta)
chv = np.cos(thetav)
